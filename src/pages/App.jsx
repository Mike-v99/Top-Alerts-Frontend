...
const THEMES = {
  paper: {
    ...
    border: "#d0c8b8", text: "#1a1200", textMid: "#6a6050", textFaint: "#aaa090",
    accent: "#8a6a00", accentBg: "rgba(138,106,0,0.1)", accentBorder: "rgba(138,106,0,0.25)",
    green: "#1a8a44", greenGlow: "none", red: "#cc2222", redGlow: "none",
    ...
    planInactiveText: "#aaa090", sparkStroke: "#8a6a00", icon: "☀",
  },
  charcoal: {
    bg: "#141414", bgCard: "#1a1a1a", bgDeep: "#0f0f0f", bgModal: "#1a1a1a", bgInput: "#0f0f0f",
    border: "#282828", text: "#e0e0e0", textMid: "#888", textFaint: "#444",
    accent: "#cccccc", accentBg: "rgba(255,255,255,0.06)", accentBorder: "rgba(255,255,255,0.15)",
    green: "#3ddc84", greenGlow: "0 0 8px #3ddc84", red: "#ff5a5a", redGlow: "0 0 8px #ff5a5a",
    ...
    planInactiveText: "#444", sparkStroke: "#aaa", icon: "◑",
  },
};

const MARKET_SYMBOLS = [
  { id: "DJI",    label: "Dow 30",  symbol: "^DJI"     },
  { id: "SPX",    label: "S&P 500", symbol: "^GSPC"    },
  { id: "VIX",    label: "VIX",     symbol: "^VIX"     },
  { id: "IXIC",   label: "Nasdaq",  symbol: "^IXIC"    },
  { id: "DXY",    label: "DXY",     symbol: "DX-Y.NYB" },
  { id: "BTCUSD", label: "BTC/USD", symbol: "BTC-USD"  },
...
const DELIVERY = [
  { id: "push",    label: "Push",    pro: false },
  { id: "email",   label: "Email",   pro: false },
  { id: "sms",     label: "SMS",     pro: true  },
  { id: "webhook", label: "Webhook", pro: true  },
];
...
export default function AppPage() {
  ...
  async function handleSaveAlert() {
    const tv =
      form.trigger?.input === "price"   ? { price: parseFloat(form.value) }
    ...
    const { error, upgrade } = await createAlert({
      asset: form.asset, asset_type: form.asset.includes("/") ? "crypto" : "stock",
      trigger_type: form.trigger.id, trigger_value: tv,
      delivery: form.delivery, cooldown_mins: parseInt(form.cooldown),
    });

    if (upgrade) { setShowModal(false); setTab("pricing"); showToast("Upgrade to Pro to unlock this feature", "warn"); return; }
    ...
    <div style={{ minHeight: "100vh", background: T.bg, ...font, color: T.text, position: "relative", overflowX: "hidden", transition: "background 0.3s" }}>
      ...
      {toast && (
        <div style={{ position: "fixed", top: 20, right: 20, zIndex: 200,
          background: toast.type === "success" ? T.btnPrimary : toast.type === "warn" ? "#c89000" : T.red,
          color: T.btnText, padding: "12px 20px", borderRadius: 10, ...font, fontSize: 18 }}>
          {toast.msg}
        </div>
      )}

      <div style={{ position: "relative", zIndex: 1, maxWidth: 980, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {/* Pro banner */}
        {isPro && (
          <div style={{ background: T.proGradient, border: `1px solid ${T.accentBorder}`, borderRadius: 12, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
                ...
                return (
                  <div key={m.id} onClick={() => openModal(m.label)} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ ...font, fontSize: 15, color: T.text }}>{m.label}</span>
                      {d && <span style={{ ...mono, fontSize: 9, color: col }}>
                        {up ? "▲" : "▼"}{Math.abs(d.changePct || 0).toFixed(1)}%
                      </span>}
                    </div>
                    <div style={{ ...mono, fontSize: 10, color: T.textMid, marginTop: 2 }}>
                      {marketLoading && !d ? "..." : fmt(d?.price)}
                    </div>
                  </div>
                );
              })}
              <div style={{ ...mono, fontSize: 8, color: T.textFaint, textAlign: "center", marginTop: 4, letterSpacing: 1 }}>
                LIVE · 60S REFRESH
              </div>
            </div>
          </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {MARKET_SYMBOLS.map(m => {
                    ...
                    return (
                      <div key={m.id} style={{
                        background: T.bgCard, border: `1px solid ${T.border}`,
                        borderLeft: `4px solid ${col}`, borderRadius: 11, padding: "16px 18px",
                      }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div>
                            <div style={{ ...font, fontSize: 22, color: T.text }}>{m.label}</div>
                            <div style={{ ...mono, fontSize: 9, color: T.textFaint, marginTop: 1 }}>{m.symbol}</div>
                          </div>
                          {d && <div style={{ textAlign: "right" }}>
                            <div style={{ ...mono, fontSize: 10, color: col }}>
                              {up ? "▲" : "▼"} {Math.abs(d.changePct || 0).toFixed(2)}%
                            </div>
                        ...
                        <div style={{ ...font, fontSize: 28, color: T.text, marginTop: 8 }}>
                          {marketLoading && !d ? "Loading..." : fmt(d?.price)}
                        </div>
                        {d?.prevClose && (
                          <div style={{ ...mono, fontSize: 9, color: T.textFaint, marginTop: 4 }}>
                            Prev close: {fmt(d.prevClose)}
                          </div>
                        )}
                        <button onClick={() => openModal(m.label)} style={{
                          marginTop: 12, padding: "5px 14px", background: "none",
                ...
                {history.map((h, i) => (
                  <div key={i} style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 11, padding: "18px 22px", display: "flex", alignItems: "center", gap: 16, position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      {h.price_at_fire && <div style={{ ...mono, fontSize: 11, color: T.accent, marginTop: 2 }}>{fmt(h.price_at_fire)}</div>}
                    </div>
                  </div>
                ))}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
                  {FREE_TRIGGERS.map(t => (
                    <button key={t.id} onClick={() => { setForm(f => ({ ...f, trigger: t })); setStep(2); }} style={{
                      ...
                      <div><div>{t.label}</div><div style={{ ...mono, fontSize: 10, color: T.textFaint }}>{t.desc}</div></div>
                    </button>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {PRO_TRIGGERS.map(t => (
                    <button key={t.id} onClick={() => {
                      ...
                      setForm(f => ({ ...f, trigger: t })); setStep(2);
                    }} style={{
                      padding: "11px 16px", borderRadius: 9,
                      border: `1px solid ${form.trigger?.id === t.id ? T.accent : T.border}`,
                      background: form.trigger?.id === t.id ? T.accentBg : T.bgDeep,
                      color: isPro ? T.text : T.border, cursor: "pointer", ...font, fontSize: 17,
                      textAlign: "left", display: "flex", gap: 10, alignItems: "center",
                    }}>
