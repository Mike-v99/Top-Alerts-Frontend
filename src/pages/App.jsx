      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          ...
          <div style={{ background: T.bgModal, border: `1px solid ${T.border}`, borderRadius: 18, width: "100%", maxWidth: 540, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 40px 80px rgba(0,0,0,0.25)" }}>
            ...
            {/* Modal header — style #10 graphite + blue accent */}
            <div style={{ padding: "18px 24px", borderBottom: `1px solid #252525`, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#1a1a1a", zIndex: 10, borderRadius: "18px 18px 0 0" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 26, height: 26, borderRadius: "50%", border: "2px solid #378ADD", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#e0e0e0", flexShrink: 0 }}>
                  {step}
                </div>
                <div>
                  <div style={{ fontSize: 20, color: "#e0e0e0" }}>{step === 1 ? "Choose Trigger" : step === 2 ? "Configure" : "Delivery"}</div>
                  <div style={{ display: "flex", gap: 4, marginTop: 5 }}>
                    {[1,2,3].map(s => <div key={s} style={{ width: s <= step ? 20 : 6, height: 3, borderRadius: 2, background: s <= step ? "#378ADD" : "rgba(255,255,255,0.12)", transition: "all 0.3s" }} />)}
                  </div>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.35)", cursor: "pointer", fontSize: 22 }}>×</button>
            </div>
            ...
            {step === 1 && (
              <div style={{ padding: "22px 28px" }}>
                ...
                {/* Card source: style #10 graphite inner card + blue accent */}
                {modalSource === "card" && (
                  <div style={{ border: "1px solid #252525", borderRadius: 10, overflow: "hidden", marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, background: "#1a1a1a", padding: "12px 16px" }}>
                      <div style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(55,138,221,0.15)", border: "1px solid rgba(55,138,221,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ ...mono, fontSize: 10, color: "#378ADD", fontWeight: 700 }}>{form.asset?.slice(0,3)}</span>
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ ...font, fontSize: 16, fontWeight: 500, color: "#e0e0e0" }}>{form.asset}</div>
                        <div style={{ ...mono, fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>{modalAssetLabel !== form.asset ? modalAssetLabel : ""}</div>
                      </div>
                      <div style={{ ...mono, fontSize: 9, color: "#378ADD", border: "1px solid rgba(55,138,221,0.35)", padding: "3px 8px", borderRadius: 4, flexShrink: 0 }}>SELECTED</div>
                    </div>
                    {modalPrice && (
                      <div style={{ background: "#111", borderTop: "1px solid #252525", padding: "7px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ ...font, fontSize: 14, fontWeight: 500, color: "#e0e0e0" }}>${Number(modalPrice.price).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        {modalPrice.marketOpen
                          ...
                          : <span style={{ ...mono, fontSize: 9, color: "#666", border: "1px solid #333", borderRadius: 3, padding: "1px 6px" }}>CLOSED</span>
                        }
                      </div>
                    )}
                  </div>
                  ...
                  {FREE_TRIGGERS.map(t => {
                    ...
                    return (
                      <button key={t.id} onClick={() => { if (disabled) return; setForm(f => ({ ...f, trigger: t })); setStep(2); }} style={{
                        ...
                        <div style={{ width: 32, height: 32, borderRadius: 7, background: iconBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 16, color: iconCol }}>
                          {t.icon}
                        </div>
                        <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {PRO_TRIGGERS.map(t => (
                    <button key={t.id} onClick={() => { if (!isPro) { setShowModal(false); setTab("pricing"); showToast("Pro plan required", "warn"); return; } setForm(f => ({ ...f, trigger: t })); setStep(2); }} style={{
                      ...
                      <div style={{ width: 32, height: 32, borderRadius: 7, background: T.bgDeep, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 14, color: T.textFaint }}>
                        {t.icon}
                      </div>
            ...
            {step === 2 && form.trigger && (
              <div style={{ padding: "22px 28px" }}>
                <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, overflow: "hidden", marginBottom: 22 }}>
                  <div style={{ background: "#1a1a1a", padding: "14px 18px", display: "flex", gap: 12, alignItems: "center" }}>
                    <div style={{ width: 40, height: 40, borderRadius: 8, background: "rgba(55,138,221,0.15)", border: "1px solid rgba(55,138,221,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
                      {form.trigger.icon}
                    </div>
                    <div>
                      <div style={{ ...mono, fontSize: 10, color: "rgba(255,255,255,0.35)" }}>{form.asset}</div>
                      <div style={{ fontSize: 18, color: "#e0e0e0", marginTop: 2 }}>{form.trigger.label}</div>
                    </div>
                  </div>
                  {modalPrice && (
                    <div style={{ background: "#111", borderTop: "1px solid #252525", padding: "7px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ ...font, fontSize: 14, fontWeight: 500, color: "#e0e0e0" }}>${Number(modalPrice.price).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      {modalPrice.marketOpen
                        ? <span style={{ ...mono, fontSize: 10, color: modalPrice.changePct >= 0 ? T.green : T.red }}>{modalPrice.changePct >= 0 ? "▲" : "▼"} {Math.abs(modalPrice.changePct).toFixed(2)}% · {modalPrice.changePct >= 0 ? "+" : ""}{Number(modalPrice.change).toFixed(2)}</span>
                        : <span style={{ ...mono, fontSize: 9, color: "#666", border: "1px solid #333", borderRadius: 3, padding: "1px 6px" }}>CLOSED</span>
