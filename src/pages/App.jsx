...
export default function AppPage() {
  ...
  const [mobileCardOrder, setMobileCardOrder] = useState(() => {
            ...
            <div>
              ...
              {!calLoading && (
                <div style={{ overflow: "hidden" }}>
                  ...
                  {/* Detail panel below — glass style */}
                  <div style={{
                    ...
                    borderRadius: isMobile ? 16 : 14, padding: isMobile ? "16px 16px" : "22px 28px",
                    boxShadow: T.cardShadow,
                  }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <div style={{ ...mono, fontSize: 10, color: T.textFaint, letterSpacing: "2px" }}>
                        EVENTS · {new Date(yr, mo, calSelectedDay).toLocaleDateString("en-US", { month: "long", day: "numeric" }).toUpperCase()}
                      </div>
                      <div style={{ ...font, fontSize: 16, fontWeight: 500, color: T.text }}>
                    ...
                    {selEvents.length === 0 && (
                      <div style={{ padding: "24px 0", textAlign: "center", color: T.textFaint, ...font, fontSize: 14 }}>No events on this day — click a day above</div>
                    )}
                    ...
                    {selEvents.length > 0 && (() => {
                      ...
                      return Object.entries(groups).map(([type, items]) => {
                        ...
                        return (
                        <div key={type} style={{ borderBottom: `1px solid ${T.border}` }}>
                          {/* Accordion header */}
                          <div onClick={() => setCalCollapsed(p => ({ ...p, [type]: !p[type] }))} style={{
                            ...
                            background: isOpen ? col + "08" : "transparent", margin: "0 -16px", padding: "14px 16px",
                            ...
                            <span style={{ ...font, fontSize: 12, color: isOpen ? col : T.textFaint }}>{isOpen ? "▲ Collapse" : "▼ Expand"}</span>
                          </div>
                          ...
                          {!isOpen && (
                            <div style={{ display: "flex", gap: 6, padding: "0 0 12px", flexWrap: "wrap" }}>
                              {previewItems.map((e, j) => (
                                <span key={j} style={{ ...mono, fontSize: 11, color: T.textMid, background: T.accentBg, padding: "4px 10px", borderRadius: 6, border: `1px solid ${T.border}` }}>
                                  {e.symbol || e.label?.split(" ").slice(0, 2).join(" ")}
                                </span>
                              ))}
                              {items.length > 5 && <span style={{ ...mono, fontSize: 11, color: T.textFaint, padding: "4px 0" }}>+{items.length - 5} more…</span>}
                            </div>
                          )}
        ...
        {isMobile && tab === "market" && (
          <div>
            ...
            {[...mobileCardOrder, ...MARKET_SYMBOLS.filter(m => !mobileCardOrder.includes(m.symbol)).map(m => m.symbol)]
              ...
              return (
                <div key={m.id} id={`edit-row-${m.symbol}`} style={{ position: "relative" }}>
                <div id={`swipe-wrapper-${m.symbol}`} style={{ position: "relative", overflow: "hidden", marginBottom: 10, borderRadius: 16 }}>
            ...
            {watchlist.filter(w => !MARKET_SYMBOLS.some(m => m.symbol === w.symbol)).map(w => {
              ...
              const snap = wd || {};
              return (
                <div key={w.symbol} id={`edit-row-${w.symbol}`} style={{ position: "relative" }}>
                <div id={`swipe-wrapper-${w.symbol}`} style={{ position: "relative", overflow: "hidden", marginBottom: 10, borderRadius: 16 }}>
                  {/* Delete button revealed by swipe */}
                  <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 90, background: T.red, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "0 16px 16px 0" }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {!loading && allAlerts.filter(a => a.status !== "deleted").map((a) => {
              ...
              <div key={a.id} style={{
                ...
                {/* Collapsed row */}
                <div onClick={() => setExpandedAlert(isOpen ? null : a.id)} style={{
                  padding: "16px 14px", display: "flex", alignItems: "center", gap: 16,
                }}>
                  <div style={{ width: 10, height: 10, borderRadius: "50%", flexShrink: 0,
                    background: a.status === "triggered" ? T.red : a.status === "paused" ? T.textFaint : T.green,
          ...
          <div style={{
            ...
            <div style={{ padding: "16px 20px 30px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 4 }}>
                <span onClick={() => setDetailSymbol(null)} style={{ fontSize: 22, color: T.text, cursor: "pointer" }}>×</span>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 16px" }}>
                  {[
                    ...
                    <div key={idx} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0" }}>
                      <span style={{ ...mono, fontSize: 15, color: "rgba(255,255,255,0.5)" }}>{l}</span>
                      <span style={{ ...mono, fontSize: 15, color: l === "Change" ? col : "#fff" }}>{v}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "center", padding: isMobile ? "0" : 20 }}
          ...
          <div data-modal-scroll="true" style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: isMobile ? 0 : 18, width: "100%", maxWidth: isMobile ? "100%" : 540, maxHeight: isMobile ? "100%" : "90vh", height: isMobile ? "100%" : "auto", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {step === 1 && (
              <div style={{ padding: isMobile ? "12px 20px 20px" : "22px 28px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <span style={{ ...font, fontSize: 22, fontWeight: 400, color: T.text }}>Set Alert</span>
                  <span onClick={() => setShowModal(false)} style={{ fontSize: 32, color: "#fff", cursor: "pointer", lineHeight: 1, padding: "4px 8px" }}>×</span>
                </div>
                      ...
                      <div style={{ marginTop: 6, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 12, maxHeight: 200, overflowY: "auto" }}>
                        {modalSearchResults.map(rr => (
                          <div key={rr.symbol} onClick={() => { setForm(f => ({ ...f, asset: rr.symbol })); setModalAssetLabel(rr.name || rr.symbol); setModalSymbolSearch(""); setModalSearchResults([]); }}
                            ...
                            <div><div style={{ ...font, fontSize: 15, color: "#fff" }}>{rr.symbol}</div><div style={{ ...font, fontSize: 13, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{rr.name}</div></div>
                            <div style={{ ...font, fontSize: 12, color: "rgba(255,255,255,0.3)" }}>{rr.type}</div>
                          </div>
                        ))}
                      </div>
                    )}
